...
def _schedule_render_done_clear():
    ...
    on the next user interaction (click, select, anything)."""
    def _delayed():
        try:
...
from .converter import convert_render_image, find_render_image_path
...
# ── Matte link management (one placeholder slot → many mattes) ────────────────

class CP3D_OT_add_matte_link(Operator):
    """Add an empty matte link to a placeholder slot (pick the matte after)."""
...
class CP3D_OT_remove_matte_link(Operator):
    """Remove the matte link at *index* from the active collection."""
...
class CP3D_OT_import_mattes(Operator):
    """Import matte objects from linked sub-collections into a Matte group.

    Mirrors :class:`CP3D_OT_import_placeholders` but for meshes whose name
    contains ``matte`` (case-insensitive).  Walks the selected collection
    tree (including library-linked sub-collections and collection-instance
    Empties), creates local copies with their world transforms baked in,
    and places them in a ``Matte`` sub-collection of the selected
    collection.  No slot or material assignment.
    ...
    def execute(self, ctx):
        ...
        for item in targets:
            try:
                ...
                ok += 1
                log_add(s, 'CHECK',
                        f"{item.collection.name}: imported {count} matte(s)")
            except RuntimeError as e:
                # A missing object in the linked file (or any import failure)
                # must NOT raise a modal popup — self.report({'ERROR'}) pops a
                # dialog.  Log a red error line to the Log window instead.
                log_add(s, 'ERROR',
                        f"Import Mattes {item.collection.name}: {e}")

        if ok == 0:
            return {'CANCELLED'}
        self.report({'INFO'},
                    f"Imported {total} matte(s) across {ok} collection(s)")
        return {'FINISHED'}
...
class CP3D_OT_convert_highlight(Operator):
    ...
    def execute(self, ctx):
        s    = ctx.scene.collection_render_settings
        item = s.collections[s.active_index]
        name = bpy.path.clean_name(item.custom_name or item.collection.name)
        try:
...
class CP3D_OT_check_setup(Operator):
    ...
    Walks every ENABLED collection in the list and, for each, checks:
      ...
      - At least one active slot has one or more linked matte objects.
    ...
    Severity (drives the log colour):
      ...
      WARN (yellow)— no matte objects linked to any slot.
    ...
    def execute(self, ctx):
        ...
        for item in targets:
            col = item.collection
            display = item.custom_name.strip() or col.name
            ...
            # Matte objects — warning only (crop still works without them).
            if slots_with_object > 0 and slots_with_matte == 0:
                warns.append("no matte objects linked to any slot")
...
class CP3D_OT_export_glb(Operator):
    """Export Crop GLB for every checkmark-selected collection.

    If no collections are checkmarked, falls back to the active (highlighted)
    one so the button still works on a single collection.  Each successfully
    exported collection is marked green in the list.
    """
    ...
    def poll(cls, ctx):
        ...
        if not (0 <= s.active_index < len(s.collections)):
            return False
        # At least one selected collection, or a valid active one
        if any(i.selected and i.collection for i in s.collections):
            return True
        return s.collections[s.active_index].collection is not None
    ...
    def execute(self, ctx):
        s = ctx.scene.collection_render_settings
        # Export every checkmark-selected collection; if none are selected,
        # fall back to the active (highlighted) one.
        targets = [i for i in s.collections if i.selected and i.collection]
        if not targets and 0 <= s.active_index < len(s.collections):
            active = s.collections[s.active_index]
            if active.collection:
                targets = [active]
        if not targets:
            self.report({'WARNING'}, "No collection to export")
            return {'CANCELLED'}
...
class CP3D_OT_render_all(Operator):
    ...
    def execute(self, context):
        ...
        for item in enabled_cols:
            name = item.custom_name or item.collection.name
            col = item.collection
            ...
            if item.render_crop:
                for _sl in 'ABCDEFGHIJ':
                    ...
                    if (getattr(item, f"crop_{_sl_l}_enabled", False)
                        ...
                        render_list.append(
                            (col, name, rx, ry, f'crop_{_sl_l}', f'Crop {_sl}', item)
                        )
        ...
        for item in settings.collections:
            cname = item.custom_name or (item.collection.name if item.collection else "")
