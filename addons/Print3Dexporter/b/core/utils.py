...
def _is_matte_col(name):
    """Return True if *name* is ``Matte`` optionally followed by Blender's
    auto-numbering suffix (``.001``, ``.002``, …).

    Mirrors :func:`_is_placeholders_col` so the Matte sub-collection created by
    the matte importer is recognised even after Blender renames duplicates to
    ``Matte.001`` etc.  Single source of truth — also imported by
    placeholder_importer.py.
    """
    lower = name.lower()
    if lower == 'matte':
        return True
    if lower.startswith('matte.') and lower[6:].isdigit():
        return True
    return False


def find_matte_collection(col):
    """Return the ``Matte`` sub-collection directly under *col*, or None.

    Recognises Blender's auto-numbering (``Matte.001``, …) via
    :func:`_is_matte_col`.  Used both by the UI (to list imported mattes) and
    by the crop pass (to drive matte holdout masking).
    """
    if col is None:
        return None
...
def set_active_camera_view(context, cam):
    """Make *cam* the scene camera and switch every 3D viewport to camera view.

    No-op when *cam* is None.  Used by the 'Show' toggle so soloing a collection
    also looks through that collection's camera.
    """
    if cam is None:
        return
