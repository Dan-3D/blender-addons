...
def find_render_image_path(item):
    ...
    if not out_dir:
        return None
    name = item.collection.name if item.collection else ""
    if not name:
        return None
...
def convert_render_image(context, item, name, mode):
    ...
    return _convert_image_through_tree(context, source_path, output_path,
                                       convert_tree, tree_name)


def convert_crop_image(context, name, slot_letter):
    """Post-process *_alfa_<slot>.png through Convert_Crop → *_crop_<slot>.png.

    The crop pass renders the raw silhouette (white RGB + render alpha) and
    saves it as ``<name>_alfa_<slot>.png``.  This runs that image through the
    Convert_Crop node group — the same post-process mechanism used for Highlight
    / Shadow — to broaden the alpha edge slightly, writing the result as
    ``<name>_crop_<slot>.png``.

    Parameters
    ----------
    context     : bpy.types.Context
    name        : str  — pre-computed safe output name (e.g. "collection_01")
    slot_letter : str  — 'A'..'J' (case-insensitive)

    Returns
    -------
    str — absolute path of the written _crop_<slot>.png.

    Raises
    ------
    RuntimeError on missing source, missing node tree, or unsaved blend.
    """
    if not bpy.data.filepath:
        raise RuntimeError("Save the .blend file before converting")

    sl = slot_letter.lower()
    out_dir = get_render_output_dir()
    source_path = os.path.join(out_dir, f"{name}_alfa_{sl}.png")
    if not os.path.exists(source_path):
        raise RuntimeError(
            f"Crop alpha source not found: {source_path}\n"
            "The crop (_alfa) pass must run before Convert_Crop."
        )

    ensure_convert_node_trees()
    convert_tree = bpy.data.node_groups.get(CROP_TREE_NAME)
    if not convert_tree:
        raise RuntimeError(
            f"Convert node tree '{CROP_TREE_NAME}' not found. "
            "Use 'Setup Compositor' to load it from Convert.blend."
        )

    os.makedirs(out_dir, exist_ok=True)
    output_path = os.path.join(out_dir, f"{name}_crop_{sl}.png")
    return _convert_image_through_tree(context, source_path, output_path,
                                       convert_tree, CROP_TREE_NAME)


def _convert_image_through_tree(context, source_path, output_path,
                                convert_tree, tree_label):
    """Run *source_path* through *convert_tree* and save to *output_path*.

    Copies the convert group, swaps its internal Render Layers node for an Image
    node reading *source_path*, nests the copy in a temp compositor, and triggers
    a minimal EEVEE render whose GroupOutput is saved via write_still.  Restores
    every scene setting it touches.  Returns *output_path*.
    """
    ...
    # Throwaway trees are tracked by REFERENCE (not name) so the finally block
    # removes exactly what this call created — a stale leftover with the same
    # name (from a crash / killed render) can otherwise cause a rename
    # collision, making the name-based cleanup delete the wrong tree and leak
    # a 'Convert_Crop.001'-style copy into the scene on every crop pass.
    work_tree = None
    tree = None

    # Sweep stale leftovers from a previous interrupted run up front.
    for _stale_name in (_TEMP_COMP_NAME, "_CP3D_CONV_WORK_"):
        _stale = bpy.data.node_groups.get(_stale_name)
        if _stale is not None:
            try:
                bpy.data.node_groups.remove(_stale)
            except Exception:
                pass
    ...
    try:
        ...
        work_tree.use_fake_user = False   # copy inherits the flag — drop it
        ...
        # (leftover temps from a previous failed run were swept above)
        ...
        if not grp_node.outputs:
            raise RuntimeError(
                f"Convert node tree '{tree_label}' has no output socket — "
        ...
        # Remove the temp compositor first (it references the work copy),
        # then the throwaway rewired group copy — by reference, so a name
        # collision can never make us delete the wrong tree and leak this one.
        for _throwaway in (tree, work_tree):
            if _throwaway is not None:
                try:
                    bpy.data.node_groups.remove(_throwaway)
                except Exception:
                    pass
        ...
        try:
            ...
            pass
        # Belt and braces: collapse any numbered Convert-group duplicates the
        # purge missed (fake-user copies aren't orphans, so they survive it).
        try:
            dedupe_convert_groups()
        except Exception:
            pass
