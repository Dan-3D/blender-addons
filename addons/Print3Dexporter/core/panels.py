...
class CP3D_UL_collection_list(UIList):
    ...
    Draws: [enabled toggle] [collection name/icon] [note] [selected checkbox]

    The collection icon carries the collection's Outliner colour tag
    (collection.color_tag → COLLECTION_COLOR_0X icon), so a colour picked in
    the Collection Settings panel (or in the Outliner) shows here too.
    ...
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        if self.layout_type in {'DEFAULT', 'COMPACT'}:
            ...
            if item.collection:
                # Icon priority:
                #   1. was_rendered — GREEN icon marks the "done" state
                #      (Blender's Python UI exposes no green text; alert is
                #      red-only, so a green vector icon shows completion).
                #   2. The collection's own Outliner colour tag (COLOR_01–08).
                #   3. Plain uncoloured collection icon.
                if item.was_rendered:
                    col_icon = 'COLLECTION_COLOR_04'
                elif item.collection.color_tag != 'NONE':
                    col_icon = f'COLLECTION_{item.collection.color_tag}'
                else:
                    col_icon = 'OUTLINER_COLLECTION'
                r.label(text=item.collection.name, icon=col_icon)
            else:
                r.label(text="(None)", icon='ERROR')
            # Short note (if set) — right-aligned, just left of the checkmark.
            if item.note:
                note_r = r.row(align=True)
                note_r.alignment = 'RIGHT'
                note_r.active = False   # dimmed so it reads as secondary info
                note_r.label(text=item.note)
...
class CP3D_PT_main_panel(Panel):
    ...
    def draw(self, context):
        ...
        # export operator's target selection (every ENABLED collection, same
        # left checkmark as Render) and counts one GLB per exportable
        # placeholder slot in each.  Shown in () on the button like Render (N).
        _glb_targets = [i for i in settings.collections if i.enabled and i.collection]
        ...
        # The finish-state tint goes on a sub-row so ONLY the Render + GLB
        # button lights up — Check Setup shares the row and must stay neutral.
        # (The state auto-clears a few seconds later; see operators.py
        # _schedule_render_done_clear.)
        btn_render = row.row(align=True)
        if settings.render_done:
            btn_render.alert = True   # highlighted finish-state look
            btn_render.operator("cp3d.render_and_export",
        ...
        if 0 <= settings.active_index < len(settings.collections):
            ...
            if item.collection:
                ...
                # Output file names come straight from the collection name —
                # there is no separate "Output Name" field anymore.
                # Short note shown in the collection list (left of the checkmark).
                box.prop(item, "note", text="Note")
                # Collection colour — the SAME property as the Outliner's
                # right-click > Color Tag (collection.color_tag), so picking a
                # colour here colours the collection in the Outliner too, and
                # vice versa.  expand=True draws the swatches in one row.
                clr_row = box.row(align=True)
                clr_row.label(text="Color:")
                clr_row.prop(item.collection, "color_tag", text="", expand=True)
                box.separator()
...
_MANUAL_SECTIONS = [
    ...
    ("Quick start", [
        ...
        "3. Select a row, set Compositor and",
        "   World, then tick the Render Outputs",
        "   you want.  Output files are named",
        "   after the collection itself.",
        "4. For crops: open Placeholder Objects,",
        "   Import Placeholders + Import Holdouts,",
        "   assign slots A–J, set Width / Height",
        "   and link holdouts to slots.",
    ...
    ("Collection list", [
        ...
        "   minus its placeholders & holdouts,",
    ...
    ("Collection Settings", [
        ...
        "   (output files use the collection name)",
        ...
        "only — placeholders & holdouts hidden.",
    ]),
    ("Convert Node Trees", [
        "•  Setup Compositor — load the",
        "   Convert_Highlight / _Shadow / _Crop",
        "   groups from Convert.blend.",
        "•  Reset Highlight / Shadow — reload a",
        "   group fresh from Convert.blend,",
        "   restoring its default curves.",
        "•  A red Reset button = that group is",
        "   missing from the scene; clicking",
        "   reloads it.  If loading fails the",
        "   current group is kept + error logged.",
        "•  Duplicate groups (Convert_Crop.001…)",
    ...
    ("Placeholder Objects", [
        ...
        "•  Import Holdouts — pull in 'holdout*'",
        "   (or legacy 'matte*') meshes into the",
        "   Holdout sub-collection.  Holdouts",
        "   were called 'Mattes' before — old",
        "   scenes, names and 'Matte' collections",
        "   keep working and are renamed on import.",
        "•  Per slot: pick by selection or by",
        "   viewport eyedropper, set Width /",
        "   Height,  + / -  to add / remove.",
        "•  Holdout (per slot): Add Holdout to",
        "   link one or more holdouts — each masks",
        "   out the placeholder on its Crop pass;",
        "   other holdouts hidden.",
        "•  Render hides placeholders & holdouts",
    ...
    ("Outputs", [
        ...
        "   <name>_<letter>.glb  (+ size JSON)",
    ]),
    ("Tips", [
        "•  Each collection needs a camera.",
        "•  Render needs a Compositor assigned.",
        "•  Output names come from the collection",
        "   — rename it to change the file names.",
...
class CP3D_PT_crop_settings(Panel):
    ...
    def draw(self, ctx):
        ...
        # Import Holdouts — parallel workflow to placeholders.  Scans linked
        # sub-collections for meshes with 'holdout' (or legacy 'matte') in
        # their name and drops local copies into a 'Holdout' sub-collection.
        # No slot assignment.  A holdout only masks when LINKED to a
        # placeholder slot via the per-slot 'Holdout' dropdown below (see
        # operators.py crop pass).
        row = layout.row(align=True)
        row.operator("cp3d.import_mattes", icon='MATERIAL')

        # List of the holdout objects currently imported into this collection's
        # 'Holdout' sub-collection.  Reads the live contents so it always
        # reflects reality (hand-deleted holdouts drop off the list).  Each row
        # carries a Select button that selects that holdout in the viewport /
        # Outliner — mirroring the placeholder slots' Select button.
        ...
        if matte_col is not None:
            ...
            if matte_objs:
                mbox = layout.column(align=True)
                mbox.label(text=f"Holdouts ({len(matte_objs)}) — link to a slot below:")
                for o in matte_objs:
                    r = mbox.row(align=True)
                    r.label(text=o.name, icon='MOD_MASK')
                    op = r.operator("cp3d.select_matte", text="Select",
        ...
        for i in range(item.crop_slot_count):
            ...
            # ── Linked holdout object(s) — shown ABOVE Width / Height ─────────
            # A slot can link several holdouts; each masks out the placeholder
            # on this slot's Crop render (all other holdouts hidden).  Each
            # row searches the 'Holdout' (legacy 'Matte') sub-collection.
            ...
            if matte_col is not None:
                for li, link in enumerate(item.matte_links):
                    ...
                    lr.prop_search(link, "name", matte_col, "all_objects",
                                   text="Holdout", icon='MOD_MASK')
                    rm = lr.operator("cp3d.remove_matte_link", text="", icon='X')
                    rm.index = li
                add = mcol.operator("cp3d.add_matte_link",
                                    text="Add Holdout", icon='ADD')
                add.slot = sl
            else:
                dis = mcol.row()
                dis.enabled = False
                dis.label(text="Holdout: import holdouts first", icon='MOD_MASK')

            # Width / Height for this placeholder — written to the GLB-folder JSON
