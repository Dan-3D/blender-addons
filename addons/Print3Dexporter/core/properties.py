...
# ── Holdout link ───────────────────────────────────────────────────────────
# One placeholder slot can be linked to MANY holdout objects (called "Matte"
# before v1.0.68 — the internal identifiers keep the old name so saved scenes
# stay compatible).  Each link records the slot letter (A–J) and the holdout
# object's name.  During that slot's Crop render every linked holdout is made
# renderable and masks out the placeholder; all unlinked holdouts are hidden.
# See the crop pass in operators.py and setup_matte_holdout_objects() in
# utils.py.

class CP3D_MatteLink(PropertyGroup):
    """A single placeholder-slot → holdout-object link."""
    slot: StringProperty(default="")   # 'A'..'J'
    name: StringProperty(default="")   # holdout object name (from 'Holdout' sub-col)
...
class CollectionRenderItem(PropertyGroup):
    ...
    # (custom_name / "Output Name" was removed in v1.0.67 — output file names
    # are always derived from the collection's own name now.)
    # Short free-text note shown in the collection list row (left of the
    # selection checkmark).  Purely informational — no effect on rendering.
    note:           StringProperty(
        name="Note", default="",
        description="Short note shown in the collection list "
                    "(left of the selection checkmark)",
    )
    ...
    # and integer Width / Height values (written into the GLB-folder JSON and
    ...
    # ── Per-slot holdout links (one slot → many holdouts) ────────────────────────
    # matte_links holds CP3D_MatteLink entries; each carries a slot letter and a
    # holdout object name.  A slot can have several linked holdouts — during that
    # slot's Crop render every linked holdout masks out the placeholder, and all
    # unlinked holdouts are hidden.  No links → no holdout masking.  (The
    # identifier keeps the pre-1.0.68 "matte" name for saved-scene compatibility.)
    matte_links: CollectionProperty(type=CP3D_MatteLink)

    # Legacy single-holdout fields (pre-1.0.46).  Kept so existing assignments
    # still drive the masking; new links use matte_links above.  Not shown in UI.
    crop_a_matte: StringProperty(name="Holdout", default="")
    crop_b_matte: StringProperty(name="Holdout", default="")
    crop_c_matte: StringProperty(name="Holdout", default="")
    crop_d_matte: StringProperty(name="Holdout", default="")
    crop_e_matte: StringProperty(name="Holdout", default="")
    crop_f_matte: StringProperty(name="Holdout", default="")
    crop_g_matte: StringProperty(name="Holdout", default="")
    crop_h_matte: StringProperty(name="Holdout", default="")
    crop_i_matte: StringProperty(name="Holdout", default="")
    crop_j_matte: StringProperty(name="Holdout", default="")
...
class CollectionRenderSettings(PropertyGroup):
    ...
    # placeholder & holdout sub-collections.  show_backup stores the pre-toggle
    ...
    show_isolate: BoolProperty(
        ...
        description="Show only the active (highlighted) collection in the "
                    "viewport, minus its placeholder and holdout sub-collections",
