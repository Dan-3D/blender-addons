"""Placeholder + holdout object importers for Print3Dexporter.
...
local-copy machinery:
  ...
  ``import_mattes(context, item)``
      Finds meshes with ``holdout`` (or the legacy ``matte``) in their
      name, copies them into a ``Holdout`` sub-collection of the selected
      collection.  No slot or material assignment — pure import + organise.
...
Both importers:
  ...
  3. Find or create the destination sub-collection — Blender's
     auto-numbering suffixes (``placeholders.001``, ``Holdout.001``, …)
     and the legacy ``Matte`` name are recognised so repeated imports
...
# ──────────────────────────────────────────────────────────────────────────────
# Holdout importer — same workflow as placeholders, but for objects with
# ``holdout`` (or the legacy ``matte``) in their name.  Destination
# sub-collection is named ``Holdout``.  Holdouts are not assigned to crop
# slots and do not get a material auto-assigned; the import simply collects,
# transforms and re-parents them.
# ──────────────────────────────────────────────────────────────────────────────


def _is_matte(obj):
    """Return True if *obj* is a mesh whose name (or mesh-data name) contains
    'holdout' or the legacy 'matte' (case-insensitive).

    Holdout objects were called "Matte" before v1.0.68 — source files that
    still name their meshes ``*matte*`` keep importing unchanged.
    """
    if obj.type != 'MESH':
        return False
    for needle in ('holdout', 'matte'):
        if needle in obj.name.lower():
            return True
        if obj.data and needle in obj.data.name.lower():
            return True
    return False
...
def import_mattes(context, item):
    """Import holdout objects from the selected collection into a Holdout
    sub-collection.
    ...
    Mirrors the placeholder import workflow:
      1. Recursively scan the selected collection tree (including library-
         ...
         whose name (or mesh-data name) contains ``holdout`` or the legacy
      ...
      2. Create local copies of every match, baking their accumulated
         ...
         appear in the scene.
      3. Find (or create) a ``Holdout`` sub-collection directly under the
         selected collection — auto-numbering variants (``Holdout.001``, …)
         and the legacy ``Matte`` name are recognised so repeated imports
         reuse the existing one (a legacy ``Matte`` collection is renamed to
         ``Holdout`` on reuse).
      4. Move every local copy into the ``Holdout`` sub-collection.

    Unlike placeholders there are no per-object slots and no material is
    auto-assigned — holdouts are just collected and organised.
    ...
    Returns
    -------
    int — number of holdout objects imported into the destination collection.
    """
    scene = context.scene
    col = item.collection
    ...
    # ── 1. Search the ORIGINAL collection tree for holdout objects ────────────
    found_originals = []
    _find_matte_objects(col, found_originals)

    if not found_originals:
        raise RuntimeError(
            f"No objects with 'holdout' or 'matte' in their name found "
            f"in collection '{col.name}' (searched sub-collections "
            f"and collection instances)"
        )

    # ── 2. Temporary staging collection for local copies ──────────────────────
    temp_name = "Temporary_imported_holdouts"
    temp_col = bpy.data.collections.new(temp_name)
    scene.collection.children.link(temp_col)
    ...
    try:
        ...
        # ── 4. Find or create the 'Holdout' destination sub-collection ────────
        dest_name = "Holdout"
        ...
        elif matte_col.name.lower().startswith('matte'):
            # Legacy 'Matte' sub-collection from an older scene — bring its
            # name up to date (Blender auto-suffixes if 'Holdout' is taken).
            matte_col.name = dest_name

        # ── 5. Move every holdout into the destination sub-collection ─────────
